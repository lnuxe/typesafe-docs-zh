...
> 为 GitHub 服务条款构建语义检索。在一次请求中，用一条 Choice 问题针对自然语言查询给 218 个行 ID 打分，再用一条 Noul 问题检查文档里是否含有答案。

你手上有 GitHub 的服务条款，还有一个关于它的自然语言问题。你需要能回答这个问题的那些行，还需要一种办法来判断文档其实没有答案。随附的查询会把有直接答案的行排在前面。`exists` 阈值把其余情况分成「没有」和「部分回答」。最后你得到一个 `find()`，它返回 `exists` 概率和每行一个相关度分数。
...
1. 给每一行打上 ID，这样 TypeSafe 才能指向它。
2. 用一条 `Choice` 问题，按各行回答查询的好坏给这些行 ID 排序。Choice 问题的概率之和恒为 1，所以即使没有一行回答了查询，也总会有一行排在第一。
3. 在同一次请求里，用一条 `Noul` 问题检查文档到底有没有答案。

## 环境准备

### 获取 TypeSafe API key

在 TypeSafe 控制台创建一个密钥并导出：
...
`JsonCache` 会重放随附的 API 响应，因此下面的步骤不需要 API key，也不产生任何花费。想让请求真正发出去，就设置 `TYPESAFE_API_KEY` 并删除 `json_cache.json`。

### 创建脚本

`semantic_search.py` 的开头是导入和客户端：
...
测试文档是 GitHub 的服务条款，切成 218 个条款，这样每条检索结果都指向一行可以引用的文字。

在 `semantic_search.py` 中追加：
...
缓存避免了重复下载，`splitlines()` 留下一个含 218 个字符串的列表。

现在给每一行加上一个短 ID 前缀，再把各行拼回一份文档。模型用这些 ID 指向它的答案。
...
一条 `Choice` 问题会给每个选项返回一个概率。把行 ID 当作选项，「选一个选项」就变成了「指向一行」。
...
选项描述是 `None`，因为文档本身已经含有每个 ID 对应的文本。查询放在 `instructions` 里；多次检索之间状态保持不变。

<Info> 一条 `Choice` 问题最多接受 255 个选项，所以这个配方一次请求最多检索 255 行的文档。超过这个长度就分两趟：第一条 Choice 问题挑出一个行窗口，第二条在该窗口内给行排序。 </Info>

## 第 3 步：检查答案是否存在

Choice 的概率之和恒为 1，所以即使文档没有回答这个问题，也总会有一行排在第一。光看排序无法区分真正的答案和最接近的无关行。
...
与 Choice 的概率不同，Noul 的概率不依赖其他选项，所以文档没有答案时它可以低到接近零。

## 第 4 步：在一次请求里发出两个问题

`system_one` 方法在一次处理中回答两个问题。状态只发一次，所以加上存在性检查只需要多一点输出。
...
两段本地代码收尾：`verdict()` 把原始的 `exists` 概率变成三种状态，中间一种表示部分回答；`show()` 把 `relevance` 画成条形图，让排序在终端里也能看懂。
...
这些阈值能把下面的例子区分开，但用在生产环境之前，请拿你自己的文档调一调。

## 第 6 步：跑一次检索

问两个有直接答案的问题、一个没有答案的、一个只有部分答案的，一共四个。
...
前两条查询返回直接答案，以及核对答案所需的原文行。

另外两条说明了存在性检查为什么重要：

* **仲裁：** 排序给最接近的那一行打了 0.86 分，但 `exists` 只有 0.14。答案并不在文档里。
* **家长许可：** 年龄规则排在第一，但它没有回答家长许可会不会改变这条规则。结果是**部分回答**。

排序告诉你该去哪儿找；`exists` 分数告诉你这个结果是否回答了问题。

## 在自己的文档上试试

[在 TypeSafe Playground 中打开这份打好标签的合同][playground]，针对同一段文本编辑问题。想检索自己的文档，就替换 `fetch_document()` 里的 URL；脚本其余各行都基于 `LINES` 工作。
